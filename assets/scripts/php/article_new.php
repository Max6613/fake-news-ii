...
if (isset($_POST['title']) && !empty($_POST['title'])
    ...
    if (isset($_FILES['img'])) {
        ...
        if( !empty($_FILES['img']['name']) ) {
            ...
            //On vérifie si le fichier a été envoyé au serveur
        ...
        if (!$upload){
            header('Location: ../../../article_new.php?err=' . $err_nb);
            die();
        }
    ...
    elseif (isset($_POST['img-select']) && !empty($_POST['img-select'])) {
        $image = $_POST['img-select'];
    }

    //Sinon erreur => pas d'image
    else {
        header('Location: ../../../article_new.php?err=1');
        die();
    }
    ...
    if (!empty($error)){
        header('Location: ../../../article_new.php?err=' . $error . '&title=' . $_POST['title'] . '&chapo=' . $_POST['chapo'] . '&content=' . $_POST['content']);
    }
